...
if (cmd === "validate") {
  ...
  if (!u) { console.error(`Unité inconnue: ${id}`); process.exit(1); }
  if (u.status === "done") { console.error(`${id} déjà done`); process.exit(1); }
  if (!depsDone(u)) { console.error(`${id}: dépendances non terminées`); process.exit(1); }
  if (!(a >= 1 && a <= 3)) { console.error(`attempt invalide: ${attempt}`); process.exit(1); }
  ...
  try { sh(`gh label create escalation --color B60205 --description "Décision humaine requise" 2>/dev/null || true`); } catch { /* label déjà existant */ }
  const body = [
    `## Escalade — ${id} (${u.title})`, "",
  ...
  sh(`gh issue create --title "ESCALADE ${id}: ${reason.slice(0, 60)}" --body-file /tmp/esc.md --label escalation --assignee ${process.env.GITHUB_REPOSITORY_OWNER || ""} || gh issue create --title "ESCALADE ${id}" --body-file /tmp/esc.md`);
} else {
